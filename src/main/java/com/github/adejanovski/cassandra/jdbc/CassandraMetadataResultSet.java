...
class CassandraMetadataResultSet extends AbstractResultSet implements CassandraResultSetExtras {
     ...
     */
    CassandraMetadataResultSet(CassandraStatement statement, MetadataResultSet metadataResultSet)
            throws SQLException {
        ...
        if (currentRow.getColumnDefinitions().getType(index - 1).isCollection()) {
            try {
                return Lists.newArrayList(currentRow.getList(index - 1,
                        Class.forName(currentRow.getColumnDefinitions().getType(index - 1)
                                .getTypeArguments().get(0).getClass().getCanonicalName())));
    ...
    public List<?> getList(String name) throws SQLException {
        checkName(name);
        // return currentRow.getList(name,String.class); // TODO: a remplacer par une vraie
        // verification des types de collections
        if (currentRow.getColumnDefinitions().getType(name).isCollection()) {
            try {
                return Lists.newArrayList(currentRow.getList(name,
                        Class.forName(currentRow.getColumnDefinitions().getType(name)
                                .getTypeArguments().get(0).getClass().getCanonicalName())));
            } catch (ClassNotFoundException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
    ...
    public Map<?, ?> getMap(int index) throws SQLException {
        ...
        if (currentRow.isNull(index - 1))
            ...
            wasNull = false;
        return currentRow.getMap(index - 1, String.class, String.class); // TODO: a remplacer par
                                                                         // une vraie verification
    ...
    public Map<?, ?> getMap(String name) throws SQLException {
        ...
        return currentRow.getMap(name, String.class, String.class); // TODO: a remplacer par une
                                                                    // vraie verification des types
                                                                    // de collections
    }
    ...
    public Set<?> getSet(int index) throws SQLException {
        checkIndex(index);
        try {
            return currentRow.getSet(index - 1, Class.forName(currentRow.getColumnDefinitions()
                    .getType(index - 1).getTypeArguments().get(0).getClass().getCanonicalName()));
        } catch (ClassNotFoundException e) {
            // TODO Auto-generated catch block
            throw new SQLNonTransientException(e);
        }
    ...
    public Set<?> getSet(String name) throws SQLException {
        checkName(name);
        try {
            return currentRow.getSet(name, Class.forName(currentRow.getColumnDefinitions()
                    .getType(name).getTypeArguments().get(0).getClass().getCanonicalName()));
        } catch (ClassNotFoundException e) {
            // TODO Auto-generated catch block
            throw new SQLNonTransientException(e);
        }
    ...
    class CResultSetMetaData implements ResultSetMetaData {
        ...
        public String getColumnClassName(int column)
                throws SQLException {/*
                                      * checkIndex(column); return values.get(column -
                                      * 1).getValueType().getType().getName();
                                      */
            if (currentRow != null) {
                return currentRow.getColumnDefinitions().getType(column - 1).getName().toString();
            }
            return driverResultSet.getColumnDefinitions().asList().get(column - 1).getType()
                    .getName().toString();
        ...
        public int getColumnDisplaySize(int column) throws SQLException {
            ...
            try {
                ...
                if (currentRow != null) {
                    com.github.adejanovski.cassandra.jdbc.ColumnDefinitions.Definition col = currentRow
                            .getColumnDefinitions().asList().get(column - 1);
                    jtype = TypesMap.getTypeForComparator(col.getType().toString());
                } else {
                    Definition col = driverResultSet.getColumnDefinitions().asList()
                            .get(column - 1);
                    jtype = TypesMap.getTypeForComparator(col.getType().toString());
                }
