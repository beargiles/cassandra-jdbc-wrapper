...
public class MetadataResultSets {
    ...
    public CassandraMetadataResultSet makeTableTypes(CassandraStatement statement)
            throws SQLException {
        ...
        tableTypes.add(row);
        CassandraMetadataResultSet result = new CassandraMetadataResultSet(statement,
                new MetadataResultSet().setRows(tableTypes));
        return result;
    }

    public CassandraMetadataResultSet makeCatalogs(CassandraStatement statement)
            throws SQLException {
        final ArrayList<MetadataRow> catalog = Lists.newArrayList();
        MetadataRow row = new MetadataRow().addEntry("TABLE_CAT",
                statement.connection.getCatalog());
        catalog.add(row);

        CassandraMetadataResultSet result = new CassandraMetadataResultSet(statement,
                new MetadataResultSet().setRows(catalog));
        return result;

    }

    public CassandraMetadataResultSet makeSchemas(CassandraStatement statement,
        ...
        for (KeyspaceMetadata keyspace : keyspaces) {
            if ("%".equals(schemaPattern))
                schemaPattern = null;
            if ((schemaPattern == null ? keyspace.getName() : schemaPattern)
                    .equals(keyspace.getName())) {
                MetadataRow row = new MetadataRow().addEntry("TABLE_SCHEM", keyspace.getName())
        ...
        CassandraMetadataResultSet result = new CassandraMetadataResultSet(statement,
                new MetadataResultSet().setRows(schemas));
        return result;

    }

    public CassandraMetadataResultSet makeTables(CassandraStatement statement, String schemaPattern,
        ...
        // 9. SELF_REFERENCING_COL_NAME String => name of the designated "identifier" column of a
        // typed table (may be null)
        // 10. REF_GENERATION String => specifies how values in SELF_REFERENCING_COL_NAME are
        ...
        for (KeyspaceMetadata keyspace : keyspaces) {
            ...
            if ((schemaPattern == null ? keyspace.getName() : schemaPattern)
                ...
                for (TableMetadata table : tables) {
                    if ((tableNamePattern == null ? table.getName() : tableNamePattern)
                        ...
                        MetadataRow row = new MetadataRow()
                                ...
                                .addEntry("TYPE_CAT", null).addEntry("TYPE_SCHEM", null)
    ...
    public CassandraMetadataResultSet makeColumns(CassandraStatement statement,
                ...
                for (TableMetadata table : tables) {
                    if ((tableNamePattern == null ? table.getName() : tableNamePattern)
                        ...
                        for (ColumnMetadata column : columns) {
                            if ((columnNamePattern == null ? column.getName() : columnNamePattern)
                                ...
                                MetadataRow row = new MetadataRow()
                                        ...
                                        .addEntry("IS_NULLABLE", "").addEntry("SCOPE_CATALOG", null)
        ...
        // 6.TYPE_NAME String => Data source dependent type name, for a UDT the type name is fully
        // qualified
        // 7.COLUMN_SIZE int => column size.
        // 8.BUFFER_LENGTH is not used.
        // 9.DECIMAL_DIGITS int => the number of fractional digits. Null is returned for data types
        ...
        // 13.COLUMN_DEF String => default value for the column, which should be interpreted as a
        // string when the value is enclosed in
        ...
        // YES --- if the parameter can include
        // NULLs
        ...
        // 21.SCOPE_TABLE String => table name that this the scope of a reference attribure (null if
        ...
        // 23.IS_AUTOINCREMENT String => Indicates whether this column is auto incremented - YES ---
        // if the column is auto incremented
        // - NO --- if the column is not auto incremented
        // - empty string --- if it cannot be determined whether the column is auto incremented
        // parameter is unknown
        // 24. IS_GENERATEDCOLUMN String => Indicates whether this is a generated column ï YES ---
        // if this a generated column
        // - NO --- if this not a generated column
        // - empty string --- if it cannot be determined whether this is a generated column

    }

    public CassandraMetadataResultSet makeIndexes(CassandraStatement statement, String schema,
            String tableName, boolean unique, boolean approximate) throws SQLException {
                    ...
                    if (tableName.equals(table.getName())) {
                        for (IndexMetadata index : table.getIndexes()) {
                            MetadataRow row = new MetadataRow()
                                    ...
                                    .addEntry("ASC_OR_DESC", null).addEntry("CARDINALITY", -1 + "")
                                    .addEntry("PAGES", -1 + "").addEntry("FILTER_CONDITION", null);
                            schemas.add(row);
                        }
                    }
                }
        ...
        // 7.TYPE short => index type: - tableIndexStatistic - this identifies table statistics that
        // are returned in conjuction with a table's index descriptions
        // - tableIndexClustered - this is a clustered index
        // - tableIndexHashed - this is a hashed index
        // - tableIndexOther - this is some other style of index
        //
        // 8.ORDINAL_POSITION short => column sequence number within index; zero when TYPE is
        // tableIndexStatistic
        // 9.COLUMN_NAME String => column name; null when TYPE is tableIndexStatistic
        // 10.ASC_OR_DESC String => column sort sequence, "A" => ascending, "D" => descending, may
        // be null if sort sequence is not supported; null when TYPE is tableIndexStatistic
        // 11.CARDINALITY int => When TYPE is tableIndexStatistic, then this is the number of rows
        // in the table; otherwise, it is the number of unique values in the index.
        // 12.PAGES int => When TYPE is tableIndexStatisic then this is the number of pages used for
        // the table, otherwise it is the number of pages used for the current index.
        // 13.FILTER_CONDITION String => Filter condition, if any. (may be null)

        /*
         * StringBuilder query = new
         * StringBuilder("SELECT keyspace_name, columnfamily_name, column_name, component_index, index_name, index_options, index_type FROM system.schema_columns"
         * );
         */

    }

    public CassandraMetadataResultSet makePrimaryKeys(CassandraStatement statement, String schema,
                    ...
                    if (tableName.equals(table.getName())) {
                        ...
                        for (ColumnMetadata col : table.getPrimaryKey()) {
                            MetadataRow row = new MetadataRow()
                                    ...
                                    .addEntry("KEY_SEQ", seq + "").addEntry("PK_NAME", null);
                            schemas.add(row);
                            seq++;
                        }
        ...
        // 3.TABLE_NAME String => table name
        // 4.COLUMN_NAME String => column name
        // 5.KEY_SEQ short => sequence number within primary key( a value of 1 represents the first
